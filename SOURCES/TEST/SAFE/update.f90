MODULE update
  ...
  !INTEGER, DIMENSION(4)               :: ipar= [100,20,6,0]
  !REAL(KIND=8), DIMENSION(4)          :: fpar= [1.d-6,1.d-2,0.d0,0.d0]
...
CONTAINS 
  ...
  SUBROUTINE euler(un,unext)
    ...
    IF (inputs%if_galerkin) THEN
       ...
       ELSE
          !CALL solve_splib(ipar, fpar, pre_mass, rk/lumped, ff, isolve, 1)
    ...
    IF (inputs%if_lumped) THEN
       ...
       ELSE IF (inputs%viscous_type=='type2')  THEN
          ...
          ELSE
             !unext = un+inputs%dt*rk
             unext(1,:)  = max(un(1,:)*(1 + inputs%dt*fix_roundoff/lumped) + inputs%dt*rk(1,:),0.d0)
             !unext(1,:)  = un(1,:)*(1 + inputs%dt*fix_roundoff/lumped) + inputs%dt*rk(1,:)
             !unext(1,:)  = un(1,:) + inputs%dt*fix_roundoff/lumped + inputs%dt*rk(1,:)
             unext(2:,:) = un(2:,:)+inputs%dt*rk(2:,:)
          END IF
       ELSE
    ...
    DO i = 1, mesh%np
       DO p = dij%ia(i), dij%ia(i+1) - 1
          ...
          IF (i.NE.j) THEN
             ...
             END DO
             ul = un(2:,i)*(2*un(1,i)/(un(1,i)**2+max(un(1,i),inputs%hsmall)**2))
             ur = un(2:,j)*(2*un(1,j)/(un(1,j)**2+max(un(1,j),inputs%hsmall)**2))
             !ul=un(2:,i)/max(un(1,i),inputs%hsmall)
             !ur=un(2:,j)/max(un(1,j),inputs%hsmall)
  ...
  END SUBROUTINE smb_1
   ...
   SUBROUTINE friction(un,rk)
    ...
    INTEGER :: d, i, j, k, p
    REAL(KIND=8) :: fric
    REAL(KIND=8), PARAMETER :: alpha = 2.d0, eta=1.d0/3.d0, chi=3.d0
...
!!$    END DO
    ...
    fric = inputs%gravity*inputs%mannings**2
    hstar = MAX(un(1,:),inputs%hsmall)**(1.d0+eta)
    !hstar = un(1,:)**(1.d0+eta)
    vel = fric*SQRT(velocity(1,:)**2+velocity(2,:)**2)
  ...
  SUBROUTINE smb_2_roundoff(un,rk)
    ...
    DO i = 1, mesh%np
       DO p = cij(1)%ia(i), cij(1)%ia(i+1) - 1
          ...
          IF (un(1,i).LE.inputs%hsmall) THEN !Important for long-time WB
             ratij=0.d0
          ELSE 
             ratij = Hstarij/un(1,i)
          END IF
          !IF (un(1,j).LE.0.d0) THEN
          IF (un(1,j).LE.inputs%hsmall) THEN !Important for long-time WB
             ratji=0.d0
          ELSE
             ratji = Hstarji/un(1,j)
          END IF
