...
CONTAINS
  ...
  FUNCTION flux(un) RESULT(vv)
    ...
    REAL(KIND=8), DIMENSION(inputs%syst_size,2,mesh%np):: vv
    ...
    CASE(1,2,3,4,5,6,7,8,9)
       ...
       DO n = 1, mesh%np
          ...
          !un(3,n) = velocity(2,n)*un(1,n) !It makes Arnau's test long-time stable on the fine grid.
          !===Limit velocity
          vv(1,1,n) = velocity(1,n)*un(1,n)
          vv(1,2,n) = velocity(2,n)*un(1,n)
          vv(2,1,n) = velocity(1,n)*un(2,n)
          vv(2,2,n) = velocity(2,n)*un(2,n)
          vv(3,1,n) = velocity(1,n)*un(3,n)
          vv(3,2,n) = velocity(2,n)*un(3,n)
  ...
  SUBROUTINE compute_one_over_h(un)
    USE mesh_handling
    IMPLICIT NONE
    REAL(KIND=8), DIMENSION(:,:) :: un
    INTEGER :: n
    DO n = 1, mesh%np
       one_over_h(n) = (2*un(1,n)/(un(1,n)**2+max(un(1,n),inputs%hsmall)**2))
    END DO
  END SUBROUTINE compute_one_over_h
  ...
  SUBROUTINE init(un)
    ...
    INTEGER :: i, k
    REAL(KIND=8) :: ray1, ray2, scale, h0, a, hL, eta, omega, &
         hcone, rcone, scone, htop, radius
    !===For malpasset
    ...
    CASE(1) !Arnau's case (revisited)
       inputs%gravity=1.d0
       inputs%hsmall=inputs%epsilon*0.2
       DO i = 1, mesh%np
    ...
    CASE(2,3,6,7) !Bump 3.1 (3.1.3 sub, 3.1.4 trans, 3.1.5 critical)
       ...
       inputs%hsmall=inputs%epsilon*0.2d0 !2
       inputs%hsmall=inputs%epsilon*2.d0 !3
       inputs%hsmall=inputs%epsilon*2.d0 !6
       inputs%hsmall=inputs%epsilon*2.d0 !7
       DO i = 1, mesh%np
    ...
    CASE(4) !Dam break wet/dry, 4.1.2 (Delestre)
       ...
       bath = 0.d0
       hL=0.005d0
       inputs%hsmall=inputs%epsilon*hL
    ...
    CASE(8) !Solitary wave around an island
       ...
       hcone=0.9d0
       h0=0.32
       inputs%hsmall=inputs%epsilon*h0
       DO i = 1, mesh%np
    ...
    CASE(9) ! Malpasset
       ...
       CLOSE(30)
       inputs%hsmall=inputs%epsilon*MAXVAL(bath_old)
  ...
  FUNCTION sol_anal(k,rr,t) RESULT(vv) !CAREFUL HERE, rr comes form the calling subroutine
    USE mesh_handling
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: k
    ...
    REAL(KIND=8) :: x, x0, speed, q0, hL, b, d, x1, x2, x3, &
         ...
         alpha, beta, chi, vel, xs, hcone, htop, radius, rcone, scone
    !===Malpasset
    ...
    CASE(1)
       ...
       CASE(1)
          DO i = 1, SIZE(rr,2)
             vv(i)  = MAX(MAX(bath(i),0.2d0)-bath(i),0.d0)  !+ inputs%hsmall
          END DO
       CASE DEFAULT
          vv(:) = 0.d0
       END SELECT
    CASE(2) ! 3.1.2
       ...
       CASE(1)
          DO i = 1, SIZE(rr,2)
             vv(i) = MAX(MAX(bath(i),0.1d0)-bath(i),0.d0) + inputs%hsmall
          END DO
       CASE DEFAULT
          vv(:) = 0.d0
       END SELECT
       ...
       CASE(1) ! water height
          DO i = 1, SIZE(rr,2)
             ...
             vv(i) = MAX((eta*h0/a**2)*(2.d0*(rr(1,i)-hL/2.d0)*COS(omega*t) &
                  +2.d0*(rr(2,i)-hL/2.d0)*SIN(omega*t)) - bathi,0.d0) !+ inputs%hsmall
          END DO
       CASE(2)
          DO i = 1, SIZE(rr,2)
             bathi = -h0*(1.d0-((rr(1,i)-hL/2.d0)**2+(rr(2,i)-hL/2.d0)**2)/a**2)
...
!!$       !TEST
  ...
  SUBROUTINE compute_lambda(ul,ur,nij,lambda)
    ...
    hl = ul(1)
    hr = ur(1)
    vl =  SUM(ul(2:k_dim+1)*nij)*(2*hl/(hl**2+max(hl,inputs%hsmall)**2))
    vr =  SUM(ur(2:k_dim+1)*nij)*(2*hr/(hr**2+max(hr,inputs%hsmall)**2))
    sql = SQRT(inputs%gravity*ABS(hl))
    sqr = SQRT(inputs%gravity*ABS(hr))
    SELECT CASE(inputs%type_test)
    CASE(1,2,3,4,5,6,7,8)
       IF (hl.LE.inputs%hsmall .AND. hr.LE.inputs%hsmall) THEN
          lambda = 0.d0
          !lambda = MAX(ABS(vl - 2*sql),ABS(vr + 2*sqr))
       ELSE IF (hl.GE.inputs%hsmall .AND. hr.GE.inputs%hsmall) THEN
       ...
       ELSE IF  (hl.LE.inputs%hsmall .AND. hr.GE.inputs%hsmall) THEN
          !ht = (MAX(vl-vr+2*sql+2*sqr,0.d0))**2/(16*inputs%gravity)
          lbdl = vl - sql!*SQRT((1+max((ht-hl)/(2*hl),0.d0))*(1+max((ht-hl)/hl,0.d0)))
          lbdr = vr + sqr!*SQRT((1+max((ht-hr)/(2*hr),0.d0))*(1+max((ht-hr)/hr,0.d0)))
          lbd_dry =  vl - 2*sql
          lambda = MAX(ABS(lbdl),ABS(lbd_dry),ABS(lbdr))
       ELSE IF  (hl.GE.inputs%hsmall .AND. hr.LE.inputs%hsmall) THEN
          !ht = (MAX(vl-vr+2*sql+2*sqr,0.d0))**2/(16*inputs%gravity)
          lbdl = vl - sql!*SQRT((1+max((ht-hl)/(2*hl),0.d0))*(1+max((ht-hl)/hl,0.d0)))
          lbdr = vr + sqr!*SQRT((1+max((ht-hr)/(2*hr),0.d0))*(1+max((ht-hr)/hr,0.d0)))
          lbd_dry =  vr + 2*sqr
  ...
  SUBROUTINE compute_lambda_vacc(ul,ur,nij,lambda)
    ...
    hl = ul(1)
    hr = ur(1)
    ovhl = 2*hl/(hl**2+max(hl,inputs%hsmall)**2)
    ovhr = 2*hr/(hr**2+max(hr,inputs%hsmall)**2)
  ...
  FUNCTION phi(h,ul,hl,ur,hr) RESULT(vv)
    ...
    IF (h>hl) THEN
       ovh = (2*h/(h**2+max(h,inputs%hsmall)**2))
       ovhl = (2*hl/(hl**2+max(hl,inputs%hsmall)**2))
