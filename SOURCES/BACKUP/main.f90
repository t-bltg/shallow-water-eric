PROGRAM shallow_water
  ...
  REAL(KIND=8)                              :: hmax0
  INTEGER :: it_frame=0, delta_frame, nb_frame=1, i
  ...
  IF (inputs%type_test==8 .OR. inputs%type_test==5 .OR. inputs%type_test==9) THEN
...
!TEST
!inputs%hsmall = inputs%gravity*inputs%dt**2/10 !/h0
!TEST
...
CONTAINS
  ...
  SUBROUTINE COMPUTE_DT(u0)
    ...
    CALL compute_dij(u0)
    inputs%dt = inputs%CFL*MINVAL(lumped/ABS(dij%aa(diag)))
