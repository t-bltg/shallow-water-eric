...
CONTAINS
  ...
  SUBROUTINE st_scr_maxwell_mu_H_p_phi(H_mesh, pp_mesh, phi_mesh, INTERFACE, interface_H_mu, ia_glob, ja_glob)

    USE def_type_mesh

    IMPLICIT NONE

    TYPE(mesh_type),           INTENT(IN) :: H_mesh, pp_mesh, phi_mesh
    TYPE(interface_type),      INTENT(IN) :: INTERFACE, interface_H_mu
    INTEGER, DIMENSION(:),     POINTER    :: ia_glob, ja_glob

    INTEGER, DIMENSION(:),     POINTER    :: ia_m, ja_m, ia_p, ja_p, ia_s, ja_s, ia_b, &
         ja_b, nja_glob, nja_b, a_d, &
         ia_s1, ja_s1
    INTEGER, DIMENSION(:,:),   POINTER    :: ja_work
    INTEGER, PARAMETER :: param=150
    INTEGER :: np_m, np_p, np_s, np_b, np_glob, i_glob, j_glob, i, i_b, i_s, &
         nnz, p_i, p_f, ms, n_a_d, m1, m2, ni, nj, ki, kj, ib, jb, ci, cj, mi, mj, n1, n2, &
         i_p, m, j
    ...
    np_m = SIZE(ia_m)-1
    CALL st_csr(phi_mesh%jj, ia_s, ja_s)
    CALL st_csr(pp_mesh%jj, ia_p, ja_p)
    CALL st_csr_bloc(ia_m, ja_m, ia_b, ja_b, 3)
    NULLIFY(ia_m,ja_m)
    ...
    DO i_p = 1, np_p
       i_glob =  i_p + np_b
       nnz = ia_p(i_p+1) - ia_p(i_p)
       p_i = ia_p(i_p)
       p_f = ia_p(i_p+1)-1
       ja_work(i_glob,nja_glob(i_glob)+1:nja_glob(i_glob)+nnz) = ja_p(p_i:p_f) + np_b
       nja_glob(i_glob) =  nja_glob(i_glob) + nnz
    END DO
    ...
    DO ms = 1, interface%mes
       ...
       DO n1 = 1, H_mesh%gauss%n_w
          i = H_mesh%jj(n1,m1)
          DO n2 = 1, phi_mesh%gauss%n_w
             j_glob =  phi_mesh%jj(n2,m2) + np_b + np_p
             IF (MINVAL(ABS(ja_work(i,1:nja_glob(i))-j_glob)) == 0) CYCLE
             nja_glob(i) = nja_glob(i) + 1
             nja_glob(i+np_m) = nja_glob(i+np_m) + 1
             nja_glob(i+2*np_m) = nja_glob(i+2*np_m) + 1
    ...
    ! Block phixphi
    DO i_s = 1, np_s
       i_glob =  i_s + np_b + np_p
       nnz = ia_s(i_s+1) - ia_s(i_s)
       p_i = ia_s(i_s)
       p_f = ia_s(i_s+1)-1
       ja_work(i_glob,nja_glob(i_glob)+1:nja_glob(i_glob)+nnz) = ja_s(p_i:p_f) + np_b + np_p
       nja_glob(i_glob) =  nja_glob(i_glob) + nnz
    END DO

    NULLIFY(ia_s,ja_s,ia_p,ja_p,nja_b)
