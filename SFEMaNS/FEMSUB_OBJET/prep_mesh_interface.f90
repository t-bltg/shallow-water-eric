...
CONTAINS
  ...
  SUBROUTINE load_interface(mesh_master, mesh_slave, list_inter, INTERFACE, disjoint)

    USE def_type_mesh

    TYPE(mesh_type),           INTENT(IN)  :: mesh_master, mesh_slave
    INTEGER, DIMENSION(:),     INTENT(IN)  :: list_inter
    TYPE(interface_type),      INTENT(OUT) :: INTERFACE
...
!!$    nws_slave  = mesh_slave%gauss%n_ws
    ...
    ALLOCATE(interface%mesh1(ms),interface%mesh2(ms),&
         interface%jjs1(nws_master,ms),interface%jjs2(nws_slave,ms))


    interface%mesh1 = interface_mesh1(1:ms)
    interface%mesh2 = interface_mesh2(1:ms)
    interface%jjs1 = interface_jjs1(1:nws_master,1:ms) 
    interface%jjs2 = interface_jjs2(1:nws_slave,1:ms) 
  ...
  SUBROUTINE load_mesh_interface(mesh_master, mesh_slave, list_inter, INTERFACE)

    USE def_type_mesh

    TYPE(mesh_type),           INTENT(IN)  :: mesh_master, mesh_slave
    INTEGER, DIMENSION(:),     INTENT(IN)  :: list_inter
    TYPE(mesh_type_interface), INTENT(OUT) :: INTERFACE
    ...
    DO n = 1, mesh_slave%nps
       ...
       n_count = n_count + 1
    END DO
    ALLOCATE(interface%list_slave_node(n_count))
    n_count = 0
    DO n = 1, mesh_slave%nps
       IF (virgin_node_inter(n)) CYCLE
       n_count = n_count + 1
       interface%list_slave_node(n_count) = mesh_slave%j_s(n)
    END DO
    ...
    DO m = 1, mesh_slave%me
       ...
       END DO
       IF (nn/=0) THEN
          interface%me = interface%me + 1
          interface_slave_elem_temp(interface%me) = m

       END IF
    END DO

    ALLOCATE(interface%slave_elem(interface%me))
    ALLOCATE(interface%master_node(nw_slave,interface%me), &
         interface%slave_node (nw_slave,interface%me))

    interface%slave_elem = interface_slave_elem_temp(1:interface%me)

    interface%slave_node = mesh_slave%jj(:,interface%slave_elem)

    interface%master_node = interface_master_node_temp(:,interface%slave_elem)
