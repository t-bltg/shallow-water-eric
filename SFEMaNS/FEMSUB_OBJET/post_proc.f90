MODULE mesh_interpolation
  ...
  PUBLIC :: coupe, mesh_interp, list_interp
  PRIVATE
...
CONTAINS
  ...
  SUBROUTINE list_interp(mesh,new_rr,field,new_field,new)
    USE def_type_mesh
    IMPLICIT NONE
    TYPE(mesh_type)                          :: mesh
    REAL(KIND=8), DIMENSION(:,:), INTENT(IN ):: new_rr
    ...
    IF (new) THEN
       new_np = SIZE(new_rr,2)
       ALLOCATE(m_goal(new_np))
       r_init = new_rr(:,1)
...
100 FORMAT(2(e11.5,2X))
  ...
  SUBROUTINE mesh_interp(mesh,new_mesh,field,new_field,new)
    ...
    REAL(KIND=8), DIMENSION(2)               :: r_init, r_goal
    INTEGER,      DIMENSION(new_mesh%np)     :: m_goal
    REAL(KIND=8) ::  valeur
    INTEGER :: m, n, m_init
    LOGICAL :: ok
    LOGICAL, SAVE :: once = .false.
    ...
    IF (new) THEN
       ...
       DO n = 2, new_mesh%np
          ...
          r_init(2) = SUM(mesh%rr(2,mesh%jj(1:3,m_goal(n))))/3
          m_init = m_goal(n)
          !write(*,*) ' m_goal',m_goal(n), 1.*n/new_mesh%np,  r_goal
       END DO
       once = .TRUE.
    END IF
...
100 FORMAT(2(e11.5,2X))
  ...
  SUBROUTINE find_next(mesh, r_goal, r_old, face_old, m_old, r_next, face_next, m_next)
    ...
    !   END IF
    !END DO
    r_old = r_old +  1.d-10*((rv(:,1)+rv(:,2)+rv(:,3))/3 -r_old)
...
100 FORMAT(3(e11.5,3x),i5)
  ...
  FUNCTION inside(mesh,m,r) 
    ...
    REAL(KIND=8), DIMENSION(2) :: r1, r2, r3, &
         n1, n2, n3 
    REAL(KIND=8) :: eps_ref=0.d0, s1, s2, s3, epsilon=-1.d-14
    r1 = mesh%rr(:,mesh%jj(1,m))
    r2 = mesh%rr(:,mesh%jj(2,m))
    r3 = mesh%rr(:,mesh%jj(3,m))
