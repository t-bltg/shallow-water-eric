...
!============================================================================================
  ...
  FUNCTION Hexact(type, rr, m, mu_H_field, t) RESULT(vv) 
    ...
    vv(:) = 0.d0
!! CI Random impaire en z 2e essai + 3e essai avec changemt de stab(:)
!    r = rr(1,:)
!    z = rr(2,:)
!
!    PI = ACOS(-1.d0)
...
!IF (m.ne.0) THEN
!  DO i=1,SIZE(rr,2)
!    IF ((r(i).ge.ri).AND.(r(i).le.ro)) THEN
!
!         IF (type == 3) THEN
!           CALL RANDOM_NUMBER(vv(i))
!           vv(i) = 1.d-4*vv(i)*SIN(k0*z(i))
!         ENDIF
!       
!    ENDIF
!!  ENDDO
...
!    IF ((r(i).ge.ri).AND.(r(i).le.ro)) THEN
!
!      f(i)  = (ri-r(i))**2*(ro-r(i))**2
!      fp(i) = -2.d0*((ri-r(i))**2*(ro-r(i))+(ri-r(i))*(ro-r(i))**2)
!      amp_h = 1d-4
!
!     IF (m.ne.0) THEN
!        CALL RANDOM_NUMBER(vv(i))
!         IF (type == 1) THEN
!            vv(i) = m*vv(i)*f(i)/r(i)*SIN(k0*z(i))
!         ELSEIF (type == 4) THEN
!            vv(i) = -vv(i)*fp(i)*SIN(k0*z(i))
!         ENDIF
!
!      vv(i) = amp_h * vv(i)
!     ENDIF
...
! 5e essai
  ...
  DO i=1,SIZE(rr,2)
    IF ((r(i).ge.ri).AND.(r(i).le.ro)) THEN
     ...
     IF (m.ne.0) THEN
         IF (type == 1) THEN
            vv(i) = m*f(i)/r(i)*SIN(k0*z(i))
         ELSEIF (type == 4) THEN
            vv(i) = -fp(i)*SIN(k0*z(i))
         ENDIF
