...
CONTAINS
  ...
  SUBROUTINE three_level_ns(time, dt, Re, list_mode, pp_mesh, &
    ...
    REAL(KIND=8)                             :: moyenne, taux_precession
    ...
    tps = user_time(dummy)
    uext = 2*un-un_m1
    CALL smb_cross_prod_gauss_sft_par(vv_mesh,list_mode,uext,rotv_v,.TRUE.)
    !   CALL smb_cross_prod_gauss_sft_par(vv_mesh,list_mode,uext,rotv_v)
    IF (SIZE(chmp_mag,1)>1) THEN
       CALL smb_cross_prod_gauss_sft_par(vv_mesh,list_mode,chmp_mag,rotb_b,.FALSE.)
...
! cas PRECESSION 28/07/09
  SUBROUTINE smb_cross_prod_gauss_sft_par(mesh,list_mode,V_in,V_out,precession)
    !=================================
    ...
    INTEGER,                      POINTER       :: me
    REAL(KIND=8)   :: ray
    REAL(KIND=8)   :: user_time, tps, dummy, taux_precession, angle_s_pi, PI
    ...
    IF (once) THEN
       ...
       ELSE
          ...
          READ (21, *) 
          ! cas PRECESSION 28/07/09
          READ (21, *) taux_precession, angle_s_pi
          CLOSE(21)
       END IF
...
!             !precession sur l'axe e_x
!             Omega(:,1,i) = taux_precession
!             Omega(:,4,i) = -1.d0*taux_precession
!             WRITE(*,*) 'taux_precession =',taux_precession
!             !precession sur l'axe e_z avec list_mode(i) == 0
!             !omega(:,5,i) = 1.d0/Ro
!          ENDIF
!       ENDDO
     DO i=1, SIZE(list_mode)
        IF (list_mode(i) == 1) THEN
           ...
           omega(:,1,i) = taux_precession * SIN(angle_s_pi*PI)
           omega(:,4,i) = -taux_precession * SIN(angle_s_pi*PI)
           WRITE(*,*) 'taux_precession,angle_s_pi =',taux_precession,angle_s_pi
           WRITE(*,*) 'om_rc_m1, om_th_s_m1 =', omega(1,1,i), omega(10,4,i)
           WRITE(*,*) 'om_m1 =', omega(2,:,i)
        ELSE IF (list_mode(i) == 0) THEN
           !precession selon un axe penche d un angle angle_s_pi*PI
           omega(:,5,i) = taux_precession * COS(angle_s_pi*PI)
           WRITE(*,*) 'om_m0 =', omega(2,:,i)
        ENDIF
     ENDDO
    ENDIF ! fin du once
    ...
    DO i = 1, SIZE(list_mode)
       ...
       DO m = 1, me
          ...
          DO l = 1, l_G
             ...
             Om(index,1,i) = SUM(Omega_s(:,1)*ww(:,l))
             Om(index,3,i) = SUM(Omega_s(:,3)*ww(:,l))
             Om(index,5,i) = SUM(Omega_s(:,5)*ww(:,l))

             Om(index,2,i) = SUM(Omega_s(:,2)*ww(:,l))
             Om(index,4,i) = SUM(Omega_s(:,4)*ww(:,l))
             Om(index,6,i) = SUM(Omega_s(:,6)*ww(:,l))
    ...
    IF (.NOT.precession) THEN
       Om=0.d0
    END IF
    !
    ! cas PRECESSION 28/07/09
    !attention, -F= 2 Om x U au lhs
    RotV = RotV + 2.d0 * Om
