PROGRAM fft_parallele
  ...
  REAL(KIND=8), ALLOCATABLE, DIMENSION(:)  :: col_angle1, col_angle2
  ...
  !pour l'envoie
  INTEGER        :: proc_recep, proc_cible, b_inf, b_sup, indice, choix_fft
...
!autres modes, d'ou une dimension 2*mod_max + 1 pour le tableau 'angle'
 ...
 !attention, theta = 0 correspond a i=0 !
  ...
  !write(*,*) 'mode',mode,'col_angle1 =',col_angle1
  !write(*,*) 'mode',mode,'col_angle2 =',col_angle2
...
!INITIALISATION POUR TEST FFT phys -> spec -----------------------
  ELSE
...
!attention au mode 0 qui decale tout ...
  ...
  ELSE
     ...
     DO i=1, 2*mode_max+2
        sous_prod(i) = vect_phys(1)*col_angle1(i)+ &
                    vect_phys(2)*col_angle2(i)
     ENDDO
     res(2*mode+1) = sous_prod(2*mode+1)
     res(2*mode+2) = sous_prod(2*mode+2)
...
!la fft pour les angles locaux a chaque proc
  DO i=0,mode_max 
      ...
      ELSE
        ...
        indice = 2*i+1
        vect_spec(1) = vect_spec(1) + res(indice)
        !vect_phys(1) = vect_phys(1) + sous_prod(indice)
        indice = 2*i+2
        vect_spec(2) = vect_spec(2) + res(indice)
        !vect_phys(2) = vect_phys(2) + sous_prod(indice)
  ...
  IF (choix_fft == 1) THEN
     write(*,*) mode,': f(theta_1)=',vect_phys(1),'et f(theta_2) =',vect_phys(2)
  ELSE
     write(*,*) mode,': coeff_cos=',vect_spec(1),'et coeff_sin =',vect_spec(2)
  ENDIF  
