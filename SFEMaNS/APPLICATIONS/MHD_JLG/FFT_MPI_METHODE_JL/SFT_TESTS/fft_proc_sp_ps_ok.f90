PROGRAM fft_parallele
  
  IMPLICIT NONE
  include 'mpif.h'
  INTEGER             :: code, mode_c
  REAL(KIND=8), DIMENSION(2) :: V1, V2
  CALL MPI_INIT(code)
  CALL MPI_COMM_RANK(MPI_COMM_WORLD,mode_c,code)

!INITIALISATION POUR LES TESTS
  IF (mode_c == 0) THEN
     V1(1) = 0.d0
     V1(2) = 0.d0
  ELSEIF (mode_c == 1) THEN
     V1(1) = 0.d0
     V1(2) = 0.d0
  ELSEIF (mode_c == 2) THEN
     V1(1) = 0.d0
     V1(2) = 0.d0
  ENDIF

!INIT TEST GENE
  V1(1) = 1.5d0 ; V1(2) = -1.7d0
  IF (mode_c == 0) THEN
     V1(2) = 0.d0
  ENDIF
!  write(*,*) 'fft spec - phys'
  CALL FFT_PAR(V1, V2, 2)
  V1 = 0.d0
!  write(*,*) 'fft phys - spec'
  CALL FFT_PAR(V2, V1, 1)

  write(*,*) mode_c,': V1(1) = ',V1(1),'et V1(2)=',V1(2)

  CALL MPI_FINALIZE(code)
  

CONTAINS

SUBROUTINE FFT_PAR(V_in, V_out, choix_fft)
!fft parallele spec -> phys si choix_fft = 1 et inverse sinon

  REAL(KIND=8), DIMENSION(:),  INTENT(IN)  :: V_in
  REAL(KIND=8), DIMENSION(:),  INTENT(OUT) :: V_out  
  INTEGER,                     INTENT(IN)  :: choix_fft

  INTEGER, DIMENSION(MPI_STATUS_SIZE) :: statut
  INTEGER       :: code, nb_procs, rang, n, valeur, i, j, l
  INTEGER, SAVE :: mode, mode_max
  INTEGER       :: etiquette, sol, requete0, requete1

  REAL(KIND=8), ALLOCATABLE, DIMENSION(:), SAVE  :: col_cos, col_sin
  REAL(KIND=8), ALLOCATABLE, DIMENSION(:), SAVE  :: col_angle1, col_angle2
  REAL(KIND=8), ALLOCATABLE, DIMENSION(:), SAVE  :: angle   ! valeurs des angles
  REAL(KIND=8)                           , SAVE  :: PI
  REAL(KIND=8), DIMENSION(10)                    :: tt
     
  REAL(KIND=8), ALLOCATABLE, DIMENSION(:), SAVE  :: sous_prod   !sous produits locaux   
  REAL(KIND=8), ALLOCATABLE, DIMENSION(:), SAVE  :: res  

  !pour l'envoie
  INTEGER        :: proc_recep, proc_cible, b_inf, b_sup, indice
  LOGICAL, SAVE  :: once=.true.
  
!--------END OF DECLARATION ------------------------------
  
!--------INITIALISATION----------------------------------

  tt(1) = MPI_WTIME() 
  sol = 2

  IF (once) THEN
     once = .false.
     CALL MPI_COMM_SIZE(MPI_COMM_WORLD,nb_procs,code)
     mode_max = nb_procs-1
     CALL MPI_COMM_RANK(MPI_COMM_WORLD,rang,code)
     mode = rang 
     etiquette = 100

!Pour les colonnes col_cos et col_sin, je considere que le mode 0 se rattache a une partie
!en cos et en sin pour systematiser les envois, 
!on a donc une dimension de 2*mode_max +2 pour les colonnes !
!Pour les angles, il n'y a qu'un angle pour le mode 0, et 2 pour les
!autres modes, d'ou une dimension 2*mod_max + 1 pour le tableau 'angle'

     ALLOCATE(angle(2*mode_max+1))

     ALLOCATE(col_cos(2*mode_max+2),col_sin(2*mode_max+2))
     col_cos = 0.d0 ; col_sin = 0.d0 ; angle = 0.d0

     ALLOCATE(col_angle1(2*mode_max+2),col_angle2(2*mode_max+2))
     col_angle1 = 0.d0 ; col_angle2 = 0.d0

     ALLOCATE(sous_prod(2*mode_max+2))
     sous_prod = 0.d0 

     ALLOCATE(res(2*mode_max+2))
     res = 0.d0

  !Calcul des angles qui sont les memes sur tous les processeurs
     PI = ACOS(-1.d0)
 !attention, theta = 0 correspond a i=0 !
     DO i= 1, 2*mode_max+1
        angle(i) = 2.d0*PI/(2*mode_max+1)*(i-1)
     ENDDO
  
  !Calcul des colonnes pour la fft spec -> phys
     DO i= 1, 2*mode_max+2
        IF (i == 1) THEN
           col_cos(i) = cos(mode*angle(i))
           col_sin(i) = sin(mode*angle(i))
        ELSEIF (i > 2) THEN
           col_cos(i) = cos(mode*angle(i-1))
           col_sin(i) = sin(mode*angle(i-1))
        ENDIF
     ENDDO
  !Calucl colonnes pour la fft phys -> spec
     DO i=1, 2*mode_max+2
        IF (mode == 0) THEN
           IF (i == 1) THEN
              col_angle1(i) = 0.5d0
              col_angle2(i) = 0.d0
           ELSEIF (i > 2) THEN
              IF (mod(i,2) == 0) THEN
                 indice = i/2 - 1
                 col_angle1(i) = sin(indice*angle(mode+1)) 
              ELSE
                 indice = (i+1)/2 - 1
                 col_angle1(i) = cos(indice*angle(mode+1)) 
              ENDIF
           ENDIF
        ELSE 
           IF (i == 1) THEN
              col_angle1(i) = 0.5d0
              col_angle2(i) = 0.5d0
           ELSEIF (i > 2) THEN    
              IF (mod(i,2) == 0) THEN
                 indice = i/2 - 1
                 col_angle1(i) = sin(indice*angle(2*mode))
                 col_angle2(i) = sin(indice*angle(2*mode+1))
              ELSE
                 indice = (i+1)/2 - 1
                 col_angle1(i) = cos(indice*angle(2*mode))
                 col_angle2(i) = cos(indice*angle(2*mode+1))
              ENDIF
           ENDIF
        ENDIF
     ENDDO
     col_angle1 = col_angle1 * (2.d0/(2*mode_max+1))
     col_angle2 = col_angle2 * (2.d0/(2*mode_max+1))

  ENDIF

!------END OF INITIALISATION--------------------

!calcul des sous produits locaux : 
!sous_prod(i) = c_mode * cos(mode*theta_i) + s_mode * sin(mode*theta_i)
!attention au mode 0 qui decale tout ...
 
  IF (choix_fft == 1) THEN
 
     DO i=1, 2*mode_max+2
        sous_prod(i) = V_in(1)*col_cos(i) + V_in(2)*col_sin(i)   
     ENDDO
     res(2*mode+1) = sous_prod(2*mode+1)
     res(2*mode+2) = sous_prod(2*mode+2)
 
  ELSE
  
     DO i=1, 2*mode_max+2
        sous_prod(i) = V_in(1)*col_angle1(i) + V_in(2)*col_angle2(i)
     ENDDO
     res(2*mode+1) = sous_prod(2*mode+1)
     res(2*mode+2) = sous_prod(2*mode+2)

  ENDIF

!chaque processeur envoie la partie dont il n'a pas besoin pour le calul
!de la fft et recoit la partie utile : 
!le processeur qui s'occupe du mode 'mode' garde les valeur 
!sous_prod(2*mode+1) et sous_prod(2*mode+2) et envoie le reste.

!Chaque proc envoie a chacun des autres un doublet et recoit de lui un doublet
!a mettre au meme endroit que le doubler qu'il a envoye
  
  tt(2) = MPI_WTIME() 


  IF (sol == 1) THEN
     !solution avec MPI_SENDRECV
     DO i=0, mode_max

        IF (i /= mode) THEN
           proc_cible = i
           b_inf = 2*i+1
           b_sup = 2*i+2
        
           CALL MPI_SENDRECV(sous_prod(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_cible, etiquette, &
                res(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_cible, etiquette, &
                MPI_COMM_WORLD, statut, code)
           
        ENDIF
     ENDDO

  ELSEIF (sol == 2) THEN
     !solution avec MPI_ISSEND et MPI_IRECV
     DO i=0, mode_max
        !write(*,*) 'mode',mode,'i=',i
        IF (i /= mode) THEN
           proc_cible = i
           b_inf = 2*i+1
           b_sup = 2*i+2
           CALL MPI_ISSEND(sous_prod(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_cible, etiquette, &
                MPI_COMM_WORLD, requete0, code) 
           CALL MPI_IRECV(res(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_cible, etiquette, &
                MPI_COMM_WORLD, requete1, code) 
            
        ENDIF
     ENDDO
     
  ENDIF

  CALL MPI_WAIT(requete0, statut, code)
  CALL MPI_WAIT(requete1, statut, code)
  

  tt(3) = MPI_WTIME()
  !write(*,*) 'temps communication mode',mode,' = ', tt(3) - tt(2) 

!Il faut maintenant additionner les valeurs du vecteurs sous_prod pour obtenir 
!la fft pour les angles locaux a chaque proc
  DO i=0,mode_max 

     indice = 2*i+1
     V_out(1) = V_out(1) + res(indice)
     !vect_phys(1) = vect_phys(1) + sous_prod(indice) 
     indice = 2*i+2
     V_out(2) = V_out(2) + res(indice) 
     !vect_phys(2) = vect_phys(2) + sous_prod(indice)
    
  ENDDO

  tt(4) = MPI_WTIME()
  !write(*,*) 'Temps total mode',mode,' = ',tt(4) - tt(1) 
  !write(*,*) 'mode',mode,'temps_comm / temps_tot = ',(tt(3)-tt(2))/(tt(4)-tt(1))

END SUBROUTINE FFT_PAR

END PROGRAM fft_parallele
   
