...
!autres modes, d'ou une dimension 2*mod_max + 1 pour le tableau 'angle'
 ...
 !attention, theta = 0 correspond a i=0 !
  ...
  DO i= 1, 2*mode_max+2
     ...
     ENDIF
  ENDDO
  !write(*,*) 'mode',mode,'col_cos=',col_cos
  !write(*,*) 'mode',mode,'col_sin=',col_sin 
...
!attention au mode 0 qui decale tout ...
  ...
  !write(*,*) 'avant envoie : mode',mode,'sous_prod=',sous_prod
...
!a mettre au meme endroit que le doubler qu'il a envoye
  ...
  DO i=0, mode_max
   !  write(*,*) 'moi mode',mode,'i=',i
     IF (i /= mode) THEN
        proc_cible = i
        b_inf = 2*i+1
        b_sup = 2*i+2
    !    write(*,*) 'moi mode',mode,'proc_cible=',proc_cible,&
        !'b_inf=',b_inf,' et b_sup=',b_sup
  ...
  !      CALL MPI_SEND(sous_prod(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_cible, etiquette, &
  !         MPI_COMM_WORLD, code)  
  !      CALL MPI_RECV(sous_prod(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_recep, etiquette, &
  !         MPI_COMM_WORLD, statut, code)
  ...
  !         CALL MPI_SEND(sous_prod(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_cible, etiquette, &
  !              MPI_COMM_WORLD, code) 
  !         CALL MPI_RECV(res(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_cible, etiquette, &
  !              MPI_COMM_WORLD, statut, code) 
  !      ELSE
  !         CALL MPI_RECV(res(b_inf:b_sup), 2, MPI_DOUBLE_PRECISION, proc_cible, etiquette, &
  !              MPI_COMM_WORLD, statut, code)
...
!la fft pour les angles locaux a chaque proc
  ...
  ! write(*,*) 'apres envoie : mode',mode,'res = ', res

  !write(*,*) 'mode',mode,'calcule les sommes'  
  DO i=0,mode_max 
     indice = 2*i+1
