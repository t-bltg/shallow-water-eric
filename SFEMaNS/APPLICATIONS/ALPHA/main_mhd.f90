...
! mars 6
  ...
  REAL(KIND=8), ALLOCATABLE, DIMENSION(:)         :: check_alpha
  !-------------END OF DECLARATIONS-------------------------------------------
...
! mars 6
  ...
  CHARACTER(LEN=20)                               :: norme_u_file, norme_h_file
...
! mars 6
  ...
  !CLOSE(21)
  IF (type_pb == 'mxx' .OR. type_pb == 'mxw') THEN
     !Vous gerez vel ici
     DO i = 1, m_max_c
...
!----------------RESTART AVEC MULTIPLICATION ENERGIE MODES H IMPAIRS POUR Taylor-Couette---
  ...
  IF (type_pb == 'mxw' .OR. type_pb == 'mhd' .OR. type_pb == 'mxx') THEN
     ...
     ALLOCATE(check_alpha(H_mesh%np))
     DO i =1, H_mesh%np
       check_alpha(i) = alpha(H_mesh%rr(:,i))
     ENDDO
      IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, check_alpha,&
                                           'check_alpha.plt')
     !STOP
   DO i = 1, m_max_c
   ...
   ENDDO
     ...
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,1,2),'Hn1_init.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,2,2),'Hn2_init.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,3,2),'Hn3_init.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,4,2),'Hn4_init.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,5,2),'Hn5_init.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,6,2),'Hn6_init.plt')
     !IF (rang==0)  CALL plot_scalar_field(phi_mesh%jj, phi_mesh%rr, phin(:,1,2),'phin1_init.plt')
     !IF (rang==0)  CALL plot_scalar_field(phi_mesh%jj, phi_mesh%rr, phin(:,2,2),'phin2_init.plt')
   DO i = 1, m_max_c
     IF ((rang==0).AND.(list_mode(i)==0)) THEN
         CALL plot_scalar_field(phi_mesh%jj, phi_mesh%rr, phin(:,1,i),'phin1_init.plt')
         CALL plot_scalar_field(phi_mesh%jj, phi_mesh%rr, phin(:,2,i),'phin2_init.plt')
     ENDIF
   ENDDO
...
!**************************************************************************************
  ...
  IF (type_pb == 'mxw' .OR. type_pb == 'mhd' .OR. type_pb == 'mxx') THEN
   ...
   ENDDO
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,1,2),'Hn1.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,2,2),'Hn2.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,3,2),'Hn3.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,4,2),'Hn4.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,5,2),'Hn5.plt')
     !IF (rang==0)  CALL plot_scalar_field(H_mesh%jj, H_mesh%rr, Hn(:,6,2),'Hn6.plt')
     !IF (rang==0)  CALL plot_scalar_field(phi_mesh%jj, phi_mesh%rr, phin(:,1,2),'phin1.plt')
     !IF (rang==0)  CALL plot_scalar_field(phi_mesh%jj, phi_mesh%rr, phin(:,2,2),'phin2.plt')
  END IF 
  !---------------------------------------------------------------------------
