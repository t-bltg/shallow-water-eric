...
!============================================================================================
    ...
    IF(m==0 .and. type == 3) THEN
      DO n = 1, SIZE(rr,2)
         ...
         ELSE IF(rr(1,n) < Ri + eps) THEN
           vv(n) = 1.d0 !  t**2/(0.1+t**2) !           1.d0
         ELSEIF ((rr(2,n)>h-eps) .or. (rr(2,n)<-1.d0*eps+h)) THEN
    !       write(*,*) 'oki'
           vv(n) = 1.d0 !rr(1,n)* t**2/(0.1+t**2) !           1.d0
         END IF
      END DO
    END IF
    RETURN
  ...
  FUNCTION extension_vel(type, H_mesh, mode, t) RESULT(vv)
    USE def_type_mesh
    USE boundary_anal
    IMPLICIT NONE
    ...
    IF(mode==0 .and. type == 3) THEN
      DO n = 1, SIZE(H_mesh%rr,2)
         vv(n) = H_mesh%rr(1,n)/Ri ! t**2/(0.1+t**2)*H_mesh%rr(1,n)/Ri 
      END DO
    END IF
    RETURN
