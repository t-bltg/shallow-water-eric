...
! CN
 ...
 IF (type_pb == 'mhd' ) THEN !Initialisation des variable perturbee

      ALLOCATE(un_pert(vv_mesh%np,6,m_max_c))
      ALLOCATE(Hn_pert(H_mesh%np,6,m_max_c))
      un_pert = un
      DO i = 1, m_max_c
        IF (list_mode(i)==0) THEN
          un_pert(:,3,i) = un(:,3,i) - vv_exact(3, vv_mesh%rr, list_mode(i), time)
        END IF
     END DO
...
!----------------RESTART AVEC MULTIPLICATION ENERGIE MODES H IMPAIRS POUR Taylor-Couette---
  ...
  IF (type_pb == 'nst' .OR. type_pb == 'mhd') THEN 
     ...
     IF (rang==0)  CALL plot_scalar_field(vv_mesh%jj, vv_mesh%rr, un(:,1,1),'Un1_init.plt')
     IF (rang==0)  CALL plot_scalar_field(vv_mesh%jj, vv_mesh%rr, un(:,2,1),'Un2_init.plt')
     IF (rang==0)  CALL plot_scalar_field(vv_mesh%jj, vv_mesh%rr, un(:,3,1),'Un3_init.plt')
     IF (rang==0)  CALL plot_scalar_field(vv_mesh%jj, vv_mesh%rr, un(:,4,1),'Un4_init.plt')
     IF (rang==0)  CALL plot_scalar_field(vv_mesh%jj, vv_mesh%rr, un(:,5,1),'Un5_init.plt')
     IF (rang==0)  CALL plot_scalar_field(vv_mesh%jj, vv_mesh%rr, un(:,6,1),'Un6_init.plt')
     IF (rang==0)  CALL plot_scalar_field(pp_mesh%jj, pp_mesh%rr, phin(:,1,1),'pn1_init.plt')
     IF (rang==0)  CALL plot_scalar_field(pp_mesh%jj, pp_mesh%rr, phin(:,2,1),'pn2_init.plt')
  END IF
